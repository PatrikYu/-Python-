...
class Solution:
    ...
    def GetMedian(self,data):  # 记得将data作为一个参数传入！！！
        ...
        if length%2 == 0:
            return (self.data[length//2] + self.data[length//2-1])/2.0  # // 整数除法,返回不大于结果的一个最大的整数./是浮点数除法
        else:
...
class Solution:
    def FirstNotRepeatingChar(self, s):
        ...
        sum = 0
        for i in s:
            d[i] = s.count(i)  # 这样做不行！！！d中key的顺序是乱的。。。而且count函数过程中key甚至是重复的...
            sum += d[i]
            if d[i] == 1:
                return sum-1
        return -1
...
思路：当一个字符第一次从字符中读出时，保存在dict中，再次被读出时，将其value设为一个特殊值，如-1，始终对这个dict进行遍历找第一个value为1的
"""
...
"""
"""
链表有公共结点，意味着这两个链表从某一节点开始，他们的.next都指向同一个节点，然后由于是同一个节点，后面所有的节点也必然相同。
