...
def dfs(i,j,end_x,end_y):
    ...
    for r,c in ((i+1,j),(i-1,j),(i,j+1),(i,j-1)):
        if r>=0 and r< n and c>=0 and c< m and test[r][c]==0 and state[r][c]>state[i][j]:
            ...
            dfs(r,c)
            test[r][c]=0  # 应该在这个位置，画图，因为仅仅要把走过的路径标为1，而已经探索过的方向已经遍历过了，允许某个点
...
# 17、打印从1到最大的n位数 (最大的n位数即 pow(10,n)-1)
class Solution:
    def PrintToMax(n):
        # write code here
        max = pow(10, n)
...
class Solution:
    ...
    def match(self, s, pattern):
        ...
        if len(pattern) > 1 and pattern[1] == '*':
            if s and (s[0] == pattern[0] or pattern[0] == '.'): # 第一个字符成功匹配：2、3两种情况
...
思路1：数组中有一个数字出现的次数超过数组长度的一半，也就是说它出现的次数比其他数字出现的次数的和还要多。在遍历数组的时候
...
肯定是最终保存着的数字。最后还要在遍历数组一次，统计这个数字的频次，若检查之后发现并没有达到数组长度的一半，则返回0。
...
先随机选择一个数字，然后调整数组中数字的顺序，使得比它小的都在左边，比它大的都在左边。若下标=k，则[:k]便是最小的k个数；
...
def find_least_k_nums(alist, k):
    ...
    index = partition(alist, start, end) # 分区操作：即比base小的放左边，比base大的放右边
    while index != k:
        if index > k:
...
def partition(alist, start, end):
    ...
    while start < end:
        while start < end and alist[end] >= base: # 找到小于base的元素位置end，并将此元素放到start位置上
            end -= 1
        alist[start] = alist[end]
        while start < end and alist[start] <= base: # 找到大于base的元素位置start，并将此元素放到end位置上
            start += 1
...
注意：可以从中间开始连续！
思路：记录从第一项开始的和sum，若sum<0,则置为0，然后继续累加，记录此过程中sum的最大值。
"""
# -*- coding:utf-8 -*-
class Solution:
...
class Solution:
    def Findn(self, n):
        ...
        for i in xrange(n):       # 生成0,1,2,3.。。，10,11,12。。。实际上第n位肯定远远没到第n个数字
            sum += len(str(i))   # 一直数到第i个数字的位数之和，如果刚好等于n，则就是i的第一位
            if sum >= n: # 说明i的某一位就是我们要寻找的第n位对应的数字
                # 如果sum刚好等于n，则就是i的第一位，即i[0],sum为加上第n个数字所有位数的，为i[sum-n]
                return str(i)[sum-n]
...
若从某start开始的子串中出现了重复字符(意味着在此时的序列中)！start更新为重复的那个位置+1
...
def singleNumber1(nums):
    ans = 0
    for i in range(1,33):  # 答案上写的是 从0-31
        bit = 0
        for j in range(n):
            bit += (nums[j]>>i)&1 # 将第i位上的数字（1或0）累加
        ans |= (bit%3)<<i # 将第i位上的值设为(bit%3)
    return ans
