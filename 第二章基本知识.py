...
"""
"""
只含有一个结点的二叉树是一棵单点树（只有根结点0），注意只有根结点的树高度为0。对于二叉树，只有一个分支时，必须说明它是左分支还是右分支。
...
深度优先遍历(Depth First Search):深度优先搜索二叉树是先访问根结点，然后遍历左子树接着是遍历右子树，可以利用堆栈的先进后出的特点
"""
# -*- coding:utf-8 -*-
# class TreeNode:
#     def __init__(self, x):
...
"""
"""
设n级台阶有f(n)种跳法，第一次跳一步，此时跳法数目为f(n-1);第一次跳两步，此时跳法数目为f(n-2)。故总的跳法数目为f(n-1)+f(n-2)
n=1,f(1)=1
n=2,f(2)=2
