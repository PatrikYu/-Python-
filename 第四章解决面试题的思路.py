...
"""
"""
建立一个数据栈与辅助栈，每压入一个元素到数据栈中，将数据栈中最小的放在辅助栈中，每弹出一项，若是最小项，则将辅助栈栈顶元素弹出
"""
...
"""
"""
画图，思路更清晰：如果下一个弹出的数字刚好是栈顶数字，那么直接弹出；若不是，则先压入再弹出，若始终没找到要弹出的，说明不可能是弹出序列
# 一定要用例子自己根据程序跑一遍
"""
# -*- coding:utf-8 -*-
class Solution:
...
"""
"""
在二叉树，每个节点都有两个指向子节点的指针，双向链表中每个节点的两个指针分别指向前一个节点和后一个节点。理想上能实现二者的转换。
二叉搜索树中，左子节点的值<父节点的值，右子节点的值>父节点的值，因此转换为双向链表时，原本指向左子节点的指针调整为链表中指向前一个节点
的指针(按从小到大顺序拍嘛)。中序遍历每个节点，得到的便是从小到大排好序的。然后将他们用left和right指针相互连接起来就好了。
"""
# -*- coding:utf-8 -*-
# class TreeNode:
#     def __init__(self, x):
...
class Solution:
    ...
    def connect(self,ss,L,path):
        ...
        # 最后一次，ss[i]取到了最后一个字符，此时调用的函数ss变为''，说明递归到了最底层，此时path=之前所有字符的叠加，append到L
        if ss=='':
            L.append(path)
        else:
...
"""
举一反三！！！若面试题是按照一定的要求摆放若干个数字，则可以求出这些数字的所有排列，然后一一判断每个排列是不是满足题目给定的要求
...
若组合中包含第一个字符，则下一步在剩余的字符里选择m-1个字符，若组合里不包括第一个字符，则下一步在剩余的字符里选取m个字符。
...
# 此代码根据 https://blog.csdn.net/MDreamlove/article/details/79528836 改编而来
